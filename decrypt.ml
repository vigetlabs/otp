...
let get_mask key =
  let c1 = BatDllist.get key
  and c2 = BatDllist.get (BatDllist.next key) in
...
let key = BatDllist.of_list (String.to_list (Array.get Sys.argv 1))
  and ciphertext = String.to_list (BatIO.read_line BatIO.stdin) in
